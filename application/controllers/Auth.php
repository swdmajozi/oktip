...
class Auth extends CI_Controller
{
	  	  ...
	  	  if ($this->form_validation->run() === TRUE)
		   {
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
			{
                ...
                if(!$this->ion_auth->in_group("admin")) {
                    ...
                    redirect('dashboard/donate', 'refresh');
                }elseif($this->ion_auth->in_group("members")){
                    $admin = $this->ion_auth->user()->row();
                    $data['admin']= $admin;
                    $data['view'] = 'admin/dashboard/donate';
                   // $this->load->view('member/member_template', $data);
                     $this->load->view('admin/layout', $data);
                }else {
                    echo 'nothing';
                }
	...
	public function change_password()
	{
		...
		if ($this->form_validation->run() === FALSE)
		{
            ...
            $this->_render_page('auth/login_template', $this->data);
	...
	*/
    ...
    public function delete_user($id = 0){
        ...
        $this->session->set_flashdata('message', 'Record is Deleted Successfully!');
        redirect('auth/index');
    }
      ...
      public function group_list(){
          ...
          else
          {
             ...
             $this->data['user_groups']  = $this->ion_auth->groups()->result();
              foreach ($this->data['user_groups'] as $k => $group)
              {
                  $this->data['user_groups'][$k]->group  = $this->ion_auth->get_users_groups($group->id)->result();

              }
              $this->data['admin'] = $this->ion_auth->user()->row();
              $this->data['view'] = 'auth/group_list';
              $this->_render_page('admin/layout', $this->data);
          }
